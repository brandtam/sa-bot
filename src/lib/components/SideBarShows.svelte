...
<div class="md:w-24 p-6 px-2 md:p-6">
	{#each $sanityShows as show, index}
		{#if show.active}
			<div
				on:click|preventDefault={() => handleSelect(show._id)}
				on:keyup
				class="cursor-pointer mb-4"
			>
				<div
					class="bg-white {$clickedSanityShow._id === show._id
						? ''
						: 'opacity-40'} h-18 w-12 flex items-center justify-center text-black text-2xl font-semibold rounded-lg mb-1 overflow-hidden"
				>
					<img
						src={urlFor(show.image).width(48).height(70).fit('crop').auto('format').url()}
						alt="{show.name} Group Image"
					/>
				</div>
...
<div class="flex-none py-6">
	<div class="mb-8">
		<div class="px-4 mb-2 text-white flex justify-between items-center">
			<div
				class="border-b-2 border-grey mt-1 bg-gradient-to-r from-green-500 via-blue-500 to-green-900 bg-clip-text text-2xl font-extrabold tracking-tighter text-transparent"
			>
				{$clickedSanityShow.name}
			</div>
		</div>
		{#if $sanityMembers.length === 0}
			<div class="px-4 mb-2 text-white flex justify-between items-center">
				<div
					class="border-b-2 border-grey mt-1 bg-gradient-to-r from-green-900 via-blue-500 to-green-500 bg-clip-text text-2xl font-extrabold tracking-tighter text-transparent"
				>
					No Members
				</div>
			</div>
		{:else}
			{#each $sanityMembers as member}
				<div
					...
					$selectedMember.name
						? 'text-2xl '
						: 'text-lg hover:bg-purple-900'}"
				>
					<span
						class="rounded-full block w-2 h-2 mr-2 {member.name === $selectedMember.name
							? 'border border-white'
							: ''} "
					/>
